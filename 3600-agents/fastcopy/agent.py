...
class PlayerAgent:
    ...
    def play(self, board: board.Board, sensor_data: Tuple, time_left: Callable):
        ...
        budget = max(0.05, min(4.0, budget))
        ...
        if current_opp_lane >= 7:
            if lane_reduction >= 2:
                board_gain += 20.0
            board_gain -= 3.0 * max(0, opp_lane_after_best - 4)
        ...
        best_board_points = self._static_move_points(best_move)
        ...
        search_gain -= 6.0 * self.search_miss_streak
        ...
        if (
            ...
            or (current_opp_setup_threat >= 15 and setup_reduction >= 6 and best_prob < 0.70)
            or (current_opp_lane >= 7 and lane_reduction >= 2 and best_prob < 0.76)
    ...
    def _carpet_value(self, length: int) -> int:
        table = {0: 0, 1: -1, 2: 2, 3: 4, 4: 6, 5: 10, 6: 15, 7: 21}
    ...
    def _evaluate(self, board_obj: board.Board, belief_vec=None) -> float:
        ...
        if me.turns_left <= 8:
            value += 5.0 * score_diff
            value += 6.0 * immediate_diff
            value += 4.0 * lane_pressure_diff
            value += 2.5 * setup_diff
    ...
    def _search_expecti_threshold(
        ...
        if opp_lane >= 6 or opp_threat >= 15 or opp_setup_threat >= 15:
            threshold += 0.10
        elif opp_lane >= 5 or opp_threat >= 10 or opp_setup_threat >= 10:
            threshold += 0.05

        if best_board_points >= 4:
            threshold += 0.05
        elif best_board_points >= 1:
            threshold += 0.02

        return max(0.35, min(0.85, threshold))

    def _search_expectiminimax_gain(self, board_obj: board.Board, belief_vec, search_loc, current_eval: float) -> float:
        idx = self._loc_to_idx(search_loc)
        p_hit = float(belief_vec[idx])

        hit_value = self._search_branch_value(board_obj, belief_vec, search_loc, hit=True)
        miss_value = self._search_branch_value(board_obj, belief_vec, search_loc, hit=False)

        expected_value = p_hit * hit_value + (1.0 - p_hit) * miss_value
        return expected_value - current_eval

    def _search_branch_value(self, board_obj: board.Board, belief_vec, search_loc, hit: bool) -> float:
        branch = board_obj.get_copy()
        idx = self._loc_to_idx(search_loc)

        if hit:
            branch.player_worker.increment_points(enums.RAT_BONUS)
            branch.player_search = (search_loc, True)
            next_belief = self._spawn_prior()
        else:
            branch.player_worker.decrement_points(enums.RAT_PENALTY)
            branch.player_search = (search_loc, False)
            next_belief = belief_vec.copy()
            next_belief[idx] = 0.0
            next_belief = self._normalize(next_belief)

        branch.end_turn(0)
        branch.reverse_perspective()
        next_belief = self._predict_belief(next_belief)
    ...
    def _order_moves(self, board_obj: board.Board, moves, belief_vec=None):
        ...
        def move_key(mv):
            ...
            score += 3.0 * self._lane_threat_value(my_lane_after)
            score += 0.5 * territory_gain
            ...
            if turns_left <= 8:
                low_value = (
                    ...
                    and opp_setup_reply >= immediate_points
                    and my_lane_after < 5
    ...
    def _max_lane_from_loc(self, board_obj: board.Board, start) -> int:
        best = 0
        for direction in (enums.Direction.UP, enums.Direction.RIGHT, enums.Direction.DOWN, enums.Direction.LEFT):
            cur = start
            length = 0
            while True:
    ...
    def _best_setup_threat(self, board_obj: board.Board, enemy: bool = False) -> float:
        ...
        for mv in valid_moves:
            ...
            plan_value = immediate + 0.8 * future_roll + 0.35 * future_immediate + 0.8 * self._lane_threat_value(future_lane)
            if plan_value > best:
                best = plan_value
    ...
    def _territory_balance(self, board_obj: board.Board) -> float:
        ...
        balance += 0.5 * (self._open_neighbors(board_obj, my_loc) - self._open_neighbors(board_obj, opp_loc))
        return balance
    ...
    def _endgame_low_value_penalty(
        ...
        if immediate >= 4.0:
            return 0.0
        if threat_reduction >= 4.0:
            return 0.0
        if setup_reduction >= 4.0:
            return 0.0
        if lane_reduction >= 1.0:
            return 0.0
